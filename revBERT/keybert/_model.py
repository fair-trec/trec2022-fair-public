...
# KeyBERT
from revBERT.keybert._mmr import mmr
from revBERT.keybert._maxsum import max_sum_similarity
from revBERT.keybert._highlight import highlight_document
from revBERT.keybert.backend._utils import select_backend
...
class KeyBERT:
    ...
    def _extract_keywords_single_doc(self,
        ...
        try:
            ...
            self.review_embedding = doc_embedding
            self.candidate_embedding = candidate_embeddings
            self.candidates = candidates
            #print(self.candidate_embedding)
            
            # Guided KeyBERT with seed keywords
            if seed_keywords is not None:
                seed_embeddings = self.model.embed([" ".join(seed_keywords)])
                self.metadata_embedding = seed_embeddings
            ...
            if use_mmr:
                ...
                distances = cosine_similarity(doc_embedding, candidate_embeddings)
                        #round(float(distances[0][index]), 4))
                                    
                #keywords = [(candidates[index], self.candidate_embedding[index]) 
